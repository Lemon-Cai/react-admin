/*
 * @Author: CaiPeng
 * @Date: 2023-06-26 15:56:03
 * @LastEditors: caipeng
 * @LastEditTime: 2023-06-27 10:29:56
 * @FilePath: \React\SelectDate\packages\web\src\utils\common.ts
 * @Description: 
 */
...
import { setReloadPath, setTabs } from '@web/store/slicers/TabSlice'
 ...
 */
export const flattenRoutes = (arr: CommonObjectType<unknown>[]): CommonObjectType<any>[] => {
  return arr.reduce(
 ...
 */
export const getKeyName = (path: string = '/403'): { title: string; tabKey: string; component: React.FC | React.Component  } => {
...
export const getImgsUrl = (html?: string) => {
  ...
  if (!arr) return null
  // 获取图片地址
  const urlArr = arr.reduce((prev: any[], next) => {
    const src: RegExpMatchArray | null = next.match(srcReg)
...
export const getPermission = () =>
  store.getState().login?.UserInfo?.permission || []
...
export const customizeTimer: {
  ...
  setTimeout(cb: () => void, interval: number): number | null;
  clearTimeout(): void;
  setInterval(cb: () => void, interval: number): number | null;
  clearInterval(): void;
} = {
  intervalTimer: null,
  timeoutTimer: null,
